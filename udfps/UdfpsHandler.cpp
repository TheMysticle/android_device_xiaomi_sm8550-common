...
class XiaomiSm8550UdfpsHandler : public UdfpsHandler {
  public:
    ...
    void onAcquired(int32_t result, int32_t vendorCode) {
        ...
        if (vendorCode == 0 && result == 1) {
            LOG(INFO) << "Fingerprint acquired successfully, disabling UDFPS overlay.";

            // Reset the fingerprint press status and turn off the overlay
            setFingerDown(false);
            setFodStatus(FOD_STATUS_OFF);

            LOG(INFO) << "Fingerprint status reset, fod_press_status set to 0.";
        } else if (vendorCode == 20 || vendorCode == 22) {
            // Waiting for fingerprint authentication or enroll
